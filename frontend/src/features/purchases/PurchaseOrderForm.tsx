...
export const PurchaseOrderForm = () => {
  ...
  const [items, setItems] = useState<FormLineItem[]>([
    { keyId: 'init-row-0', productId: '', description: '', qty: 1, unit: 'Pcs', rate: 0, discount: 0, taxPercent: 18, warehouseId: '' }
  ]);
  ...
  useEffect(() => {
    ...
    setExpectedDeliveryDate(meta.expectedDeliveryDate || '');
    setWarehouseId(meta.warehouseId || '');
    setBuyer(meta.buyer || '');
    ...
    if (existingPo.items && Array.isArray(existingPo.items)) {
      setItems(existingPo.items.map((i: any, index: number) => ({
        ...
        rate: Number(i.rate),
        discount: 0, // Defaults to 0 since PO Item table doesn't store discount column separately
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const invalidRow = items.find(i => !i.productId || i.qty <= 0);
    if (invalidRow) {
      toast.error('All items must have a product selected and quantity > 0');
      return;
    }
    ...
    try {
      const payload = {
        ...
        status: isEditMode ? undefined : 'DRAFT', // Service sets defaults
        items: items.map(i => ({
          productId: i.productId,
          description: i.description,
          qty: i.qty,
  ...
  const loading = loadingVendors || loadingProducts || loadingWarehouses || loadingPo;
  ...
  return (
    <PageContainer maxWidth="7xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-surface border border-border rounded-2xl p-6 space-y-6 text-left shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="md:col-span-2">
              <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Vendor *</label>
              <select
                value={vendorId}
              ...
              >
                ...
                {vendors.map(v => (
                  <option key={v.id} value={v.id}>
                    {v.name} {v.gstin || v.gstNumber ? `(GSTIN: ${v.gstin || v.gstNumber})` : ''}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div>
              <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Expected Delivery</label>
              <input
                type="date"
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div>
              <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">PO Number *</label>
              <input
                type="text"
                value={orderNo}
                ...
                className="w-full px-4 py-2.5 bg-background border border-border rounded-xl text-sm focus:outline-none focus:border-accent font-bold"
              />
            </div>

            <div>
              <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Target Warehouse</label>
              <select
                value={warehouseId}
            ...
            <div>
              <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Buyer / Representative</label>
              <input
                type="text"
                value={buyer}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-border/50">
            ...
            <div>
              <label className="block text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Shipping Address</label>
              <textarea
                rows={2}
        ...
        {/* Dynamic Items Grid */}
        <div className="bg-surface border border-border rounded-2xl p-6 space-y-4 text-left shadow-sm">
                  ...
                  return (
                    <tr key={item.keyId} className="hover:bg-muted/10 transition-colors">
                      <td className="py-4 pr-3">
                        ...
                        {/* Live stocks information helper */}
                        {(() => {
                      ...
                      <td className="py-4 px-1">
                        <input 
                          ...
                          className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm text-right focus:outline-none focus:border-accent font-bold" 
                        />
                      </td>
                      <td className="py-4 px-1">
                        <input 
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-6 border-t border-border mt-4">
            <div className="md:col-span-2 space-y-4">
              ...
              {selectedVendor && (
                <div className="bg-muted/30 border border-border/60 rounded-xl p-4 text-xs space-y-1.5 max-w-md">
                  <p className="font-extrabold text-foreground border-b border-border pb-1 mb-2 uppercase">Supplier Information Summary</p>
                  <p className="flex justify-between"><span>Supplier:</span> <span className="font-bold text-foreground">{selectedVendor.name}</span></p>
                  <p className="flex justify-between"><span>Outstanding Balance:</span> <span className="font-bold text-red-500">₹{Number(selectedVendor.payableBalance || 0).toLocaleString('en-IN')}</span></p>
              ...
              <div className="w-full space-y-4 bg-muted/20 border border-border/60 rounded-2xl p-6 text-sm">
                ...
                {taxMode === 'CGST_SGST' && summary.cgst > 0 && (
                  <>
                    <div className="flex justify-between text-muted-foreground text-xs pl-2">
                      ...
                      <span className="font-mono">₹{summary.cgst.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                    </div>
                    <div className="flex justify-between text-muted-foreground text-xs pl-2">
                      <span>Input SGST</span>
                      <span className="font-mono">₹{summary.sgst.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                    </div>
                  </>
                )}
                ...
                {taxMode === 'IGST' && summary.igst > 0 && (
                  <div className="flex justify-between text-muted-foreground text-xs pl-2">
                    ...
                    <span className="font-mono">₹{summary.igst.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                  </div>
                )}
                ...
                {Math.abs(summary.roundOff) > 0 && (
                  <div className="flex justify-between text-muted-foreground text-xs">
                    ...
                    <span className="font-mono">{summary.roundOff > 0 ? '+' : ''}₹{summary.roundOff.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                  </div>
                )}
