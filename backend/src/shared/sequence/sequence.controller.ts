...
export class SequenceController {
  ...
  async getNextSequence(@Request() req: any, @Param('documentType') documentType: string) {
  ...
  async getConfig(@Request() req: any, @Param('documentType') documentType: string) {
    const config = await this.sequenceService.getConfig(req.user.companyId, documentType);
    return { success: true, data: config };
  }

  @Post('config/:documentType')
  async updateConfig(@Request() req: any, @Param('documentType') documentType: string, @Body() data: any) {
