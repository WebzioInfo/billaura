import { Controller, Post, Get, Param, UseGuards, Req, Body, Res, ForbiddenException, Query } from '@nestjs/common';
...
export class BackupController {
  ...
  @Post('request')
  async requestBackup(@Req() req: any, @Body() body: { type: any, isPlatform: boolean }) {
    const user = req.user;
    ...
    if (body.isPlatform && user.globalRole === 'SUPER_ADMIN') {
      ...
      throw new ForbiddenException('Only Super Admins can request platform backups');
    }
  ...
  @Get('download/:id')
  async downloadBackup(@Param('id') id: string, @Req() req: any, @Res() res: Response) {
    const user = req.user;
    ...
    if (job.companyId !== user.companyId && user.globalRole !== 'SUPER_ADMIN') {
      throw new ForbiddenException('You are not authorized to download this backup');
    }
